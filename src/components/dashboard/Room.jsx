...
import { Edit, KeyboardArrowDown, KeyboardArrowLeft } from '@mui/icons-material';
...
const Room = () => {
  ...
  async function getRoom() {
    ...
    if (!dbRoom || !dbRoom.facilitatorIds.includes(currentUser.id) || !dbRoom.reflectionIds.includes(reflectionId)) {
  ...
  return (
    <div>
      <h3><KeyboardArrowLeft onClick={() => navigate(-1)}/> {room ? room.name : null} <Edit/></h3>
      <h4>{character} / Chapter {chapter} <Button onClick={handleClick}><KeyboardArrowDown/></Button></h4>
      <Menu
        anchorEl={anchorEl}
        open={open}
        onClose={handleClose}
      >
        {
          room
            ? Object.keys(REFLECTION_ID_MAP)
              .filter(reflectionId => room.reflectionIds.includes(parseInt(reflectionId)))
              .map(reflectionId => {
                const { character, chapter } = REFLECTION_ID_MAP[reflectionId]
                return <MenuItem onClick={() => handleMenuItemClick(reflectionId)}>{character} / Chapter {chapter}</MenuItem>;
              })
            : null
        }
      </Menu>
